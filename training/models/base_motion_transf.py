...
class BaseMotionTransf(nn.Module):
    def __init__(self, config) -> None:
        ...
        self.config = config
        mconfig = config.MODEL.BASE_MOTION_TRANSF
        self.USE_EXPERTS = mconfig.USE_EXPERTS and mconfig.EXPERT_NUM > 1

        if not self.USE_EXPERTS:
            self.model = MotionTransformer(config)
        else:
            self.EXPERT_NUM = config.MODEL.EXPERT_NUM
            GATING_HIDDEN_SIZE = config.MODEL.GATING_HIDDEN_SIZE
