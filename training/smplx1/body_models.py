...
class SMPLX(SMPLH):
    ...
    def forward(
        ...
        **kwargs
        ...
        body_parts = torch.cat([jaw_pose.reshape(-1, 1, 3),
                               leye_pose.reshape(-1, 1, 3),
                               reye_pose.reshape(-1, 1, 3),
                               left_hand_pose.reshape(-1, 15, 3),
                               right_hand_pose.reshape(-1, 15, 3)], dim=1).expand(bs, -1, -1)
        ...
        shape_components = torch.cat([betas, expression.expand(batch_size, -1)], dim=-1)
